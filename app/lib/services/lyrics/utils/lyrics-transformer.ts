import type { LyricsSection, AnnotationInfo } from '../types/lyrics.types';
...
export class LyricsTransformer {
  static transform(lyrics: LyricsSection[], referents: ResponseReferents[]): TransformedLyricsBySection[] {
  ...
  private static buildAnnotationMap(referents: ResponseReferents[]): Record<string, AnnotationInfo[]> {
  ...
  private static groupLines(
    ...
    const groupedLines: Record<number, { lineIds: number[]; annotations: AnnotationInfo[] }> = {};
  ...
  private static buildTransformedLines(
    ...
    section.lines.forEach(line => {
      ...
      if (group) {
        this.processGroupedLine(group, section, transformedLines, processedIds, sequentialId);
        sequentialId++;
      } else if (line.text) {
  ...
  private static createSingleLine(line: { id: number; text: string }, sequentialId: number): TransformedLine {
    return {
      id: sequentialId,
      range: {
        start: line.id,
