...
import { createErrorInfo, logError } from '../utils/errorHandling';

// Define the payload structure for this task
interface FinishAnalysisJobPayload {
  jobId: number;
}
 ...
 */
export default async function finishAnalysisJob(
  payload: FinishAnalysisJobPayload,
  helpers: { logger: any }
) {
  const { jobId } = payload;
