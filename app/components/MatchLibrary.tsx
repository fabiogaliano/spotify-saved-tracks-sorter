...
import { matchingService } from '~/core/services'
import type { Song, Playlist, MatchResult } from '~/core/domain/Matching'
...
export function MatchLibrary({ savedTracks, playlists }: MatchLibraryProps) {
  ...
  const [matchResults, setMatchResults] = useState<Array<{ song: Song; matches: Array<{ playlist: Playlist; matchResult: MatchResult }> }>>([])
  ...
  const handleMatchLibrary = async () => {
    try {
      ...
      setError(`Error matching library: ${err instanceof Error ? err.message : String(err)}`)
    } finally {
      setIsMatching(false)
    }
  }
  ...
  const matchPlaylist = async (playlist: Playlist) => {
    try {
      ...
      alert(`Top matches for ${playlist.name || playlist.id}:\n${
        results.slice(0, 5).map(r => 
          `${r.track_info.artist} - ${r.track_info.title}: ${(r.similarity * 100).toFixed(1)}%`
        ).join('\n')
      }`)
    } catch (err) {
      console.error('Error matching playlist:', err)
      setError(`Error matching playlist: ${err instanceof Error ? err.message : String(err)}`)
    } finally {
      setIsMatching(false)
    }
  }
        ...
        <div className="mt-6">
          ...
          <div className="space-y-4">
            {matchResults.map((result) => (
              <div key={result.song.track.id} className="border border-gray-700 rounded-lg p-3">
                ...
                {result.matches.length > 0 ? (
                  <div className="mt-2 space-y-2">
                    ...
                    {result.matches.map(({ playlist, matchResult }) => (
                      <div key={playlist.id} className="flex items-center justify-between text-sm p-2 bg-gray-900 rounded">
                        <span>
                          {playlist.name || `Playlist ${playlist.id}`}
                        </span>
                        <span className={`px-2 py-1 rounded ${
                          matchResult.similarity > 0.8 ? 'bg-green-600' : 
                          matchResult.similarity > 0.6 ? 'bg-green-800' :
                          matchResult.similarity > 0.4 ? 'bg-yellow-600' : 'bg-red-600'
                        }`}>
