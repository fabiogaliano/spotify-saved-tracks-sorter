...
export async function action({ request }: ActionFunctionArgs) {
	try {
		...
		const savedPlaylist = await playlistService.createAIPlaylist(
			name,
			description,
			session.userId
		)
		...
		if (error instanceof logger.AppError) {
			...
			if (error.statusCode === 400) {
				return Response.json(
					{ error: 'Invalid playlist data. Please check your input and try again.' },
					{ status: 400 }
				)
			}
			if (error.statusCode === 403) {
				return Response.json(
					{ error: 'Insufficient permissions to create playlist.' },
					{ status: 403 }
				)
			}
