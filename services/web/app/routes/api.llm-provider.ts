...
export async function loader({ request }: LoaderFunctionArgs) {
	...
	switch (action) {
		...
		case 'getProviderStatuses':
			try {
				return {
					providerStatuses: await providerKeyService.getProviderStatuses(
						userSession.userId
					),
				}
			} catch (error) {
	...
	try {
		switch (action) {
			...
			case 'deleteKey': {
				...
				return Response.json(
					{ success: true, message: `${provider} API key deleted successfully` },
					{ status: 200 }
				)
			}
			...
			case 'validateKey': {
				...
				return Response.json(
					{ success: true, message: `${provider} API key is valid` },
					{ status: 200 }
				)
			}
			...
			case 'setActiveProvider': {
				try {
					const hasKey = await providerKeyService.hasProviderKey(
						userSession.userId,
						provider
					)
					if (!hasKey) {
					...
					return Response.json(
						{ success: true, message: `${provider} set as active provider` },
						{ status: 200 }
					)
				} catch (error) {
					return Response.json(
		...
		if (error instanceof Error) {
			if (error.message.includes('encrypted_key')) {
				errorDetails =
					'There was an issue with the encryption process. Please check your encryption configuration.'
			} else if (error.message.includes('not-null constraint')) {
				errorDetails =
					'Required data is missing. Please ensure all required fields are provided.'
			}
		}
