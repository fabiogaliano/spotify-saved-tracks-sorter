...
export function ThemedCard({
	...
	const gradientClass =
		...
		: variant === 'secondary' ?
			'bg-card-secondary border-secondary/20 hover:border-secondary/40'
		: variant === 'accent' ? 'bg-card-accent border-accent/20 hover:border-accent/40'
		: 'bg-card border-border hover:border-border/80'

	return (
		<Card className={cn(gradientClass, 'transition-colors', className)}>{children}</Card>
	)
}
...
export function ThemedCardFooter({ children, className }: ThemedCardFooterProps) {
	return (
		<CardFooter className={cn('flex justify-between', className)}>{children}</CardFooter>
	)
}
