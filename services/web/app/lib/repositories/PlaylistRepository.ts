import type {
	PlaylistRepository as IPlaylistRepository,
	Playlist,
	PlaylistInsert,
	PlaylistTrack,
	PlaylistTrackInsert,
...
class SupabasePlaylistRepository implements IPlaylistRepository {
	...
	async getPlaylistAnalysis(
		playlistId: number
	): Promise<Database['public']['Tables']['playlist_analyses']['Row'] | null> {
		const { data, error } = await getSupabase()
	...
	async updateSyncStatus(userId: number, status: SyncStatus): Promise<void> {
		const { error } = await getSupabase()
			...
			.update({
				...
				playlists_last_sync:
					status === SYNC_STATUS.COMPLETED ? new Date().toISOString() : undefined,
			})
			.eq('id', userId)
	...
	async savePlaylistTracks(playlistTracks: PlaylistTrackInsert[]): Promise<void> {
		...
		for (let i = 0; i < playlistTracks.length; i += BATCH_SIZE) {
			...
			const { error } = await getSupabase().from('playlist_tracks').insert(batch)
	...
	async getPlaylistTracks(
		playlistId: number
	): Promise<Array<{ track_id: number; spotify_track_id: string }>> {
		const { data, error } = await getSupabase()
      ...
      `
		...
		return (data || [])
			.filter(
				(row): row is typeof row & { tracks: { spotify_track_id: string } } =>
	...
	async updatePlaylistTrackCounts(
		updates: Array<{ id: number; track_count: number }>
	): Promise<void> {
	...
	async getPlaylistById(playlistId: number): Promise<Playlist | null> {
		...
		if (error) {
			if (error.code === 'PGRST116') {
				// The error code for no rows returned
				return null
			}
			throw error
		}
	...
	async updatePlaylistInfo(playlist: PlaylistUpdateInput): Promise<Playlist | null> {
		const { data, error } = await getSupabase()
			.from('playlists')
			.update({
				name: playlist.name,
