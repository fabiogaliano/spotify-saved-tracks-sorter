...
class SavedTrackRepository {
	 ...
	 */
	async getSavedTracksWithDetailsByUserId(
		userId: number
	): Promise<SavedTrackWithDetails[]> {
		const { data, error } = await getSupabase()
			.from('saved_tracks')
			.select(
				`
	 ...
	 */
	async batchUpdateSortingStatus(
		userId: number,
		updates: { trackId: number; status: SortingStatus | null }[]
	): Promise<void> {
	 ...
	 */
	async getSavedTrackSpotifyIds(
		userId: number
	): Promise<{ trackId: number; spotifyTrackId: string }[]> {
		const { data, error } = await getSupabase()
			.from('saved_tracks')
