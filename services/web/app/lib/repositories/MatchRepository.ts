...
export interface MatchRepository {
	...
	getTrackPlaylistMatches(
		trackId: number
	): Promise<Array<{ playlist_id: number; score: number; factors: any }>>
	getPlaylistTrackMatches(
		playlistId: number
	): Promise<Array<{ track_id: number; score: number; factors: any }>>
...
export class SupabaseMatchRepository implements MatchRepository {
	async saveMatchResult(
		...
		try {
			logger.debug('Saving match result', {
				trackId,
				playlistId,
				similarity: matchResult.similarity,
			})

			const { error } = await getSupabase().from('track_playlist_matches').upsert(
				{
					track_id: trackId,
			...
			throw new logger.AppError('Failed to save match result', 'match-save-failed', 0, {
				error,
			})
		}
	}

	async getTrackPlaylistMatches(
		trackId: number
	): Promise<Array<{ playlist_id: number; score: number; factors: any }>> {
		try {
			...
			if (error) {
				throw new logger.AppError('Failed to get song playlist matches', error.code, 0, {
					error,
				})
			}
			...
			throw new logger.AppError(
				'Failed to get song playlist matches',
				'match-get-failed',
				0,
				{ error }
			)
	...
	async getPlaylistTrackMatches(
		...
		try {
			...
			if (error) {
				throw new logger.AppError('Failed to get playlist song matches', error.code, 0, {
					error,
				})
			}
			...
			throw new logger.AppError(
				'Failed to get playlist song matches',
				'match-get-failed',
				0,
				{ error }
			)
	...
	async deleteMatchResult(trackId: number, playlistId: number): Promise<void> {
		try {
			...
			if (error) {
				throw new logger.AppError('Failed to delete match result', error.code, 0, {
					error,
				})
			}
