...
export const CreateAIPlaylistSchema = v.object({
	name: v.pipe(
		...
		v.maxLength(
			PLAYLIST_MAX_NAME_LENGTH,
			`Playlist name cannot exceed ${PLAYLIST_MAX_NAME_LENGTH} characters`
		)
	),
	description: v.pipe(
		...
		v.maxLength(
			PLAYLIST_MAX_DESCRIPTION_LENGTH,
			`Playlist description cannot exceed ${PLAYLIST_MAX_DESCRIPTION_LENGTH} characters`
		),
		v.startsWith(
			`${PLAYLIST_AI_PREFIX}`,
			`Description must start with "${PLAYLIST_AI_PREFIX}"`
		)
	),
})
...
export const CreateAIPlaylistInputSchema = v.object({
	...
	description: v.pipe(
		...
		v.maxLength(
			...
			`Description cannot exceed ${PLAYLIST_MAX_DESCRIPTION_LENGTH - PLAYLIST_AI_PREFIX.length} characters`
		),
		v.startsWith(
			`${PLAYLIST_AI_PREFIX}`,
			`Description must start with "${PLAYLIST_AI_PREFIX}"`
