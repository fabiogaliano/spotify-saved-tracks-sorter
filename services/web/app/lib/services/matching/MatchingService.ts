...
export class MatchingService {
	...
	private async profileFromDescription(playlist: Playlist): Promise<PlaylistProfile> {
		...
		const moods =
			playlist.emotional?.dominant_mood?.mood ?
				[playlist.emotional.dominant_mood.mood]
			:	[]
		const themes = (playlist.meaning?.core_themes?.map(t => t.name) ?? []).filter(
	...
	private async scoreSong(song: Song, profile: PlaylistProfile): Promise<MatchResult> {
		...
		if (song.analysis?.audio_features && profile.avgAudioFeatures) {
			...
			audioScore = this.calculateAudioFeatureScoreFromAnalysis(
				song.analysis.audio_features,
				profile
			)
		}
		...
		if (
			song.analysis &&
			weightedEarlyScore > MATCHING_WEIGHTS.tiers.deepAnalysisThreshold
		) {
		...
		return {
			...
			veto_reason:
				finalScore < MATCHING_WEIGHTS.scoring.vetoThreshold ?
					'Low compatibility'
				:	undefined,
		}
	}
	 ...
	 */
	private async calculateMetadataScore(
		song: Song,
		...
		if (profile.moods.length > 0 && song.analysis?.emotional?.dominant_mood) {
			...
			if (moodMatch)
				score +=
					MATCHING_WEIGHTS.scoring.mood.match - MATCHING_WEIGHTS.scoring.mood.neutral
		}
	 ...
	 */
	private getMoodTransitionScore(
		fromMood: string | undefined,
		toMood: string | undefined
	): number {
		...
		if (
			Object.values(goodTransitions).some(
				moods => moods.includes(fromMood) && moods.includes(toMood)
			)
		)
			return 0.6
	...
	private getAdaptiveWeights(
		...
		if (
			profile.method === 'learned_from_songs' &&
			song.analysis &&
	...
	private calculateAudioFeatureScoreFromAnalysis(
		...
		if (avgFeatures.tempo !== undefined) {
			factors++
			const tempoScore = 1 - Math.abs((songFeatures.tempo - avgFeatures.tempo) / 100) // Normalize by 100 BPM difference
	...
	private compareAudioFeatures(
		...
		if (
			features1.instrumentalness !== undefined &&
			features2.instrumentalness !== undefined
		) {
			score +=
				weights.instrumentalness *
				(1 - Math.abs(features1.instrumentalness - features2.instrumentalness))
			...
			if (journey && journey.length > 0) {
				...
				if (moods.length >= 2) {
					...
					if (start === end) {
						...
						['happy', 'euphoric', 'empowered'].includes(end) &&
						['sad', 'anxious', 'melancholic'].includes(start)
					) {
						journeyTypes.add('ascending')
					} else if (
						['sad', 'melancholic', 'contemplative'].includes(end) &&
	 ...
	 * Calculate average audio features
	 */
	private calculateAverageAudioFeatures(
		features: ReccoBeatsAudioFeatures[]
	): Partial<ReccoBeatsAudioFeatures> {
