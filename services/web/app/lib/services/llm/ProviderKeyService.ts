...
export class ProviderKeyService {
  ...
  async saveProviderKey(userId: number, provider: string, apiKey: string): Promise<ProviderKey> {
  ...
  async getDecryptedProviderKey(userId: number, provider: string): Promise<string | null> {
  ...
  async getProviderStatuses(userId: number): Promise<Array<{ provider: string, hasKey: boolean, isActive: boolean }>> {
