...
export class SyncService {
	...
	async syncSavedTracks(userId: number): Promise<SyncResult> {
		try {
			...
			const tracksToRemove = dbSavedTracks.filter(
				t => !spotifyTrackIds.has(t.spotifyTrackId)
			)
			let removedCount = 0
			...
			if (newSpotifyTracks.length > 0) {
				const { totalProcessed, newTracks, processedTracks } =
					await this.trackService.processSpotifyTracks(newSpotifyTracks)
				newSavedTracks = await this.trackService.saveSavedTracksForUser(
					userId,
					newSpotifyTracks,
					processedTracks
				)
			...
			if (newSavedTracks.length > 0) {
				messages.push(
					`added ${newSavedTracks.length} new song${newSavedTracks.length === 1 ? '' : 's'}`
				)
			}
			if (removedCount > 0) {
				messages.push(
					`removed ${removedCount} unliked song${removedCount === 1 ? '' : 's'}`
				)
			}
			...
			throw new logger.AppError('Failed to sync tracks', 'DB_SYNC_ERROR', 500, {
				cause: error,
				userId,
			})
		}
	...
	async syncPlaylists(userId: number): Promise<PlaylistSyncResult> {
		...
		try {
			...
			throw new logger.AppError('Failed to sync playlists', 'DB_SYNC_ERROR', 500, {
				cause: error,
				userId,
			})
		}
	...
	async syncPlaylistTracks(
		...
		try {
			...
			if (playlistIds && playlistIds.length > 0) {
				...
				logger.info('fetched specific playlists', {
					count: dbPlaylists.length,
			...
			for (const dbPlaylist of dbPlaylists) {
				...
				await this.playlistService.updatePlaylistTracksStatus(
					dbPlaylist.id,
					'IN_PROGRESS'
				)
				...
				const lastSyncTime = await this.playlistService.getPlaylistTracksLastSyncTime(
					dbPlaylist.id
				)
				...
				if (needsTrackSync) {
					try {
						const trackChanges = await this.playlistService.syncPlaylistTracks(
							spotifyPlaylist,
							dbPlaylist,
							userId
						)
			...
			const totalTracksAdded = playlistTrackChanges.reduce(
				(sum, change) => sum + change.addedCount,
				0
			)
			const totalTracksRemoved = playlistTrackChanges.reduce(
				(sum, change) => sum + change.removedCount,
				0
			)
			...
			throw new logger.AppError('Failed to sync playlist tracks', 'DB_SYNC_ERROR', 500, {
				cause: error,
				userId,
			})
		}
