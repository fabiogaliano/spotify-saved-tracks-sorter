...
function calculateCollaboratorBonus(
	...
	const hasMatch = collaborators.artists.some(expected =>
		checkArray.some(
			actual => calculateSimilarity(expected.toLowerCase(), actual.toLowerCase()) > 0.8
		)
	)
...
function normalizeForComparison(title: string): string {
	return (
		title
			...
			.replace(
				/\s*[-–—]\s*(?:\d{4}\s+)?(?:remaster|edit|live|acoustic|demo|remix|version|mono|stereo|deluxe|special|extended|original|alternate|radio|single|explicit|clean|from\s+)[^-–—]*$/i,
				''
			)
