...
export class AuthService {
	 ...
	 */
	async getUserSession(request: Request): Promise<{
		...
		try {
			...
			if (timeUntilExpiry < REFRESH_THRESHOLD_MS) {
				...
				if (!refreshPromise) {
					...
					refreshPromise = this.refreshSessionToken(user).finally(() => {
					...
					this.logger.info(
						`[AUTH] Token refresh already in progress for user ${userId}, waiting...`
					)
				}
