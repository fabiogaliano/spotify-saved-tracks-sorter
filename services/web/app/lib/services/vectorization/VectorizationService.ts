 ...
 */
import { type ModelType, vectorizationConfig } from '~/lib/config/vectorization'
import { logger } from '~/lib/logging/Logger'
import type { Playlist, SentimentScore, Song } from '~/lib/models/Matching'
...
export class DefaultVectorizationService implements VectorizationService {
	...
	async vectorizeSong(song: Song): Promise<number[]> {
		if (!song.analysis) {
			throw new logger.AppError(
				'Song analysis required for vectorization',
				'VALIDATION_ERROR',
				400
			)
		}
		...
		try {
			...
			throw new logger.AppError('Failed to vectorize song', 'VECTORIZATION_ERROR', 500, {
				cause: error,
	...
	async embed(text: string, model: ModelType = 'general'): Promise<number[]> {
		try {
			...
			throw new logger.AppError('Failed to embed text', 'VECTORIZATION_ERROR', 500, {
				cause: error,
	...
	async embedBatch(texts: string[], model: ModelType = 'general'): Promise<number[][]> {
		try {
			...
			throw new logger.AppError('Failed to batch embed', 'VECTORIZATION_ERROR', 500, {
				cause: error,
				count: texts.length,
			})
		}
