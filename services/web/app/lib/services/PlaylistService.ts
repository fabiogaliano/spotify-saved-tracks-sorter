...
import {
	Playlist,
	SpotifyPlaylistDTO,
	mapSpotifyPlaylistToPlaylistInsert,
...
import { Track, mapPlaylistTrackToTrackInsert } from '~/lib/models/Track'
import {
	PlaylistUpdateInput,
	playlistRepository,
...
import {
	CreateAIPlaylistInputSchema,
	CreateAIPlaylistSchema,
} from '~/lib/validation/playlist.validation'
...
export class PlaylistService {
	...
	async createAIPlaylist(
		...
		const finalDescription =
			description.startsWith(PLAYLIST_AI_PREFIX) ? description : (
				`${PLAYLIST_AI_PREFIX}${userInput.description}`
			)
	...
	async updatePlaylistInfo(
		userId: number,
		playlistUpdate: PlaylistUpdateInput
	): Promise<Playlist> {
		try {
			...
			if (
				shouldBeAIFlagged &&
				playlistUpdate.description.trim() !== '' &&
				!playlistUpdate.description.startsWith(PLAYLIST_AI_PREFIX)
			) {
			...
			await this.spotifyService.updatePlaylist(
				playlist.spotify_playlist_id,
				finalDescription,
				playlistUpdate.name
			)
			const updatedPlaylist = await playlistRepository.updatePlaylistInfo(playlistUpdate)
	...
	async getSmartSortingEnabledPlaylistsWithTracks(
		userId: number
	): Promise<PlaylistWithTracks[]> {
		try {
			const smartSortingEnabledPlaylists =
				await playlistRepository.getFlaggedPlaylists(userId)
			const userPlaylistTracks =
				await playlistRepository.getPlaylistTracksByUserId(userId)
			...
			const results = await buildPlaylistsWithTracks(
				smartSortingEnabledPlaylists,
				playlistTracksMap,
				trackMap
			)
		...
		async function createTrackLookupMap(
			playlistTracks: any[]
		): Promise<Map<TrackId, Track>> {
		...
		function extractSuccessfulResults(
			results: PromiseSettledResult<PlaylistWithTracks>[]
		): PlaylistWithTracks[] {
			return results
				.filter(
					(result): result is PromiseFulfilledResult<PlaylistWithTracks> =>
	...
	async processSpotifyPlaylists(
		...
		const playlistsToUpdate = spotifyPlaylists.filter(playlist => {
			...
			const metadataChanged =
				existing.name !== playlist.name || existing.description !== playlist.description
		...
		if (playlistsToUpdate.length > 0) {
			...
			const trackCountUpdates = savedPlaylists
				.map(savedPlaylist => {
					const spotifyPlaylist = playlistsToUpdate.find(
						p => p.id === savedPlaylist.spotify_playlist_id
					)
				...
				.filter(
					(update): update is { id: number; track_count: number } => update !== null
				)

			if (trackCountUpdates.length > 0) {
				logger.info('updating track counts for new/updated playlists', {
					updates: trackCountUpdates,
				})
		...
		const newCount = savedPlaylists.filter(
			p => !existingPlaylistMap.has(p.spotify_playlist_id)
		).length
	...
	async syncPlaylistTracks(
		...
		const rawSpotifyPlaylistTracks = await this.spotifyService.getPlaylistTracks(
			spotifyPlaylist.id
		)
		...
		const validTracks = rawSpotifyPlaylistTracks.filter(
			t =>
				t.track != null &&
				t.track.id != null &&
		...
		const existingPlaylistTracks = await playlistRepository.getPlaylistTracks(
			dbPlaylist.id
		)
		const existingTrackSpotifyIds = new Map(
			existingPlaylistTracks.map(t => [t.spotify_track_id, t])
		)
		...
		if (newPlaylistTrackLinks.length > 0) {
			...
			const playlistTrackAssociations = newPlaylistTrackLinks
				.map(linkRecord => {
					const dbTrack = dbTrackRecords.find(
						t => t.spotify_track_id === linkRecord.spotify_track_id
					)
		...
		const trackIdsToRemove = existingPlaylistTracks
			.filter(
				existingTrack => !currentSpotifyTrackIds.has(existingTrack.spotify_track_id)
			)
		...
		if (newPlaylistTrackLinks.length > 0) {
			...
			const addedSpotifyTracks = spotifyPlaylistTracks
				.filter(track =>
					newPlaylistTrackLinks.some(link => link.spotify_track_id === track.track.id)
				)
				.map(track => ({
