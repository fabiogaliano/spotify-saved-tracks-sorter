...
export class ReccoBeatsService {
	...
	private async handleResponse<T>(response: Response): Promise<T> {
		if (!response.ok) {
			...
			throw new Error(
				`ReccoBeats API error: ${typeof errorBody === 'object' ? JSON.stringify(errorBody) : errorBody || response.statusText}`
			)
		}
	...
	private async getReccoBeatsId(spotifyTrackId: string): Promise<string | null> {
		try {
			return await this.limiter.run(async () => {
				const response = await fetch(`${this.baseUrl}/track?ids=${spotifyTrackId}`, {
					method: 'GET',
	...
	): Promise<Map<string, ReccoBeatsAudioFeatures>> {
