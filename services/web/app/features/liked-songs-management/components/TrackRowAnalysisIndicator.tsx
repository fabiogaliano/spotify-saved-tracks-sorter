...
const badgeStyle: Record<UIAnalysisStatus, string> = {
	analyzed:
		'bg-green-500/20 border-green-500 text-green-400 hover:bg-green-500/30 cursor-pointer',
	pending: 'bg-blue-500/20 border-blue-500 text-blue-400',
	not_analyzed:
		'bg-secondary/20 border-border text-muted-foreground hover:bg-secondary/30 cursor-pointer',
...
export const TrackRowAnalysisIndicator: React.FC<TrackRowAnalysisIndicatorProps> = ({
	...
	const handleClick = () => {
		if (status === 'analyzed') {
			if (onView) onView()
		} else if (status === 'not_analyzed' && onAnalyze) {
			onAnalyze()
	...
	return (
		<Badge
			className={`${badgeStyle[status]} flex items-center gap-1 border px-2 py-1 whitespace-nowrap`}
			onClick={status !== 'pending' ? handleClick : undefined}
		>
			{statusText[status]}
			{status === 'analyzed' && <Eye className="ml-1 h-3 w-3" />}
			{status === 'not_analyzed' && <RefreshCw className="ml-1 h-3 w-3" />}
			{status === 'pending' && <Clock className="ml-1 h-3 w-3" />}
			{status === 'failed' && <AlertCircle className="ml-1 h-3 w-3" />}
		</Badge>
	)
}
