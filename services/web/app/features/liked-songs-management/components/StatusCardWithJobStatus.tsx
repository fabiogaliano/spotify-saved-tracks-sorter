	...
	return (
		<Card className="bg-card border-border relative">
			<CardContent className="flex items-center justify-between p-4">
				...
				<div
					className={`${iconBg || 'bg-card'} relative rounded-full p-2 ${shouldShowTooltip ? 'cursor-pointer transition-all duration-300 hover:scale-105' : ''}`}
					onClick={shouldShowTooltip ? () => setOpen(!open) : undefined}
				>
					{/* Icon with glow effect */}
					<div
						className={`relative z-10 ${shouldShowTooltip ? 'drop-shadow-[0_0_8px_rgba(59,130,246,0.8)]' : ''}`}
					>
						{icon}
					</div>
          ...
          `}</style>
				...
				{open && activeJob && (
					<div
						ref={dropdownRef}
						className="animate-in slide-in-from-top-2 absolute top-full right-0 z-50 mt-2 transform duration-200"
						role="menu"
					>
						{/* Dropdown arrow */}
						<div className="bg-background border-border absolute -top-1 right-3 h-2 w-2 rotate-45 transform border-t border-l"></div>

						<div className="relative">
							<button
								onClick={handleManualClose}
								className="bg-background border-border hover:bg-muted absolute -top-2 -right-2 z-10 rounded-full border p-1 shadow-lg transition-colors"
								aria-label="Close"
							>
...
export const StatusCard = ({
	...
	<Card className="bg-card border-border">
		<CardContent className="flex items-center justify-between p-4">
			...
			<div className={`${iconBg || 'bg-card'} rounded-full p-2`}>{icon}</div>
		</CardContent>
	</Card>
)
