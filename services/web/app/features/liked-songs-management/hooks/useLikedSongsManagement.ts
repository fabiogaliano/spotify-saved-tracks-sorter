...
export function useLikedSongsManagement({ initialSongs }: UseLikedSongsManagementProps) {
	...
	const filteredTracks = useMemo(() => {
		return likedSongs.filter((track: TrackWithAnalysis) => {
			...
			// Status filter
			if (statusFilter !== 'all') {
				if (statusFilter === 'analyzed' && track.uiAnalysisStatus !== 'analyzed')
					return false
				if (statusFilter === 'not_analyzed' && track.uiAnalysisStatus !== 'not_analyzed')
					return false
				if (statusFilter === 'pending' && track.uiAnalysisStatus !== 'pending')
					return false
	...
	const selectedTracks = useCallback(() => {
		...
		return filteredTracks.filter(
			(_: TrackWithAnalysis, index: number) => rowSelection[index]
		)
	}, [filteredTracks, rowSelection])

	// Statistics
	const stats = useMemo(() => {
		const analyzed = likedSongs.filter(
			(t: TrackWithAnalysis) => t.uiAnalysisStatus === 'analyzed'
		).length
		const pending = likedSongs.filter(
			(t: TrackWithAnalysis) => t.uiAnalysisStatus === 'pending'
		).length
		const failed = likedSongs.filter(
			(t: TrackWithAnalysis) => t.uiAnalysisStatus === 'failed'
		).length
		const notAnalyzed = likedSongs.filter(
			(t: TrackWithAnalysis) => t.uiAnalysisStatus === 'not_analyzed'
		).length
		...
		return {
			...
			notAnalyzed,
			percentageAnalyzed:
				likedSongs.length > 0 ? Math.round((analyzed / likedSongs.length) * 100) : 0,
		}
	}, [likedSongs])

	// Analysis actions
	const analyzeSelectedTracks = useCallback(
		async (batchSize?: 1 | 5 | 10) => {
	...
	const updateSongAnalysisDetails = useCallback(
		(trackId: number, analysisData: Record<string, unknown>, status: string) => {
