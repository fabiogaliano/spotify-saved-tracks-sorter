...
import { ItemStatesMap, TrackBatchJob } from '~/lib/types/analysis.types'
...
export const action = async ({ request }: ActionFunctionArgs) => {
	...
	if (contentType.includes('application/json')) {
		...
		if (invalidIds.length > 0) {
			return {
				success: false,
				error: 'Invalid trackIds: all must be numbers',
				status: 400,
			}
		}
		...
		if (tracks.length !== trackIds.length) {
			...
			logger.warn('Some tracks not found', {
				missingIds,
		...
		const validatedBatchSize =
			validBatchSizes.includes(batchSize as 1 | 5 | 10) ?
				(batchSize as 1 | 5 | 10)
			:	undefined
		...
		try {
			...
			await jobPersistenceService.saveJob(
				contextJob,
				userSession.userId,
				tracks.map(t => t.id)
			)
			...
			try {
				const { results, failed } = await sqsService.enqueueBatchAnalysisJobs(
					payloads,
					batchId
				)
	...
	if (formAction === 'analyze') {
		...
		try {
			...
			logger.info('Single track analysis enqueued', {
				batchId,
				trackId,
				userId: userSession.userId,
			})
