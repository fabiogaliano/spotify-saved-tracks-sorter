...
import { Coffee, Heart, Moon, Music, Plus, Zap } from 'lucide-react'
...
const CreateSmartPlaylistModal: React.FC<CreateSmartPlaylistModalProps> = ({
	...
	const handleTemplateSelect = (
		template: (typeof PLAYLIST_TEMPLATES)[0],
		index: number
	) => {
	...
	return (
		...
		>
			<DialogTrigger asChild>
				<Button className="bg-primary hover:bg-primary/90 text-primary-foreground gap-2 border-0 transition-colors">
			...
			<DialogContent className="sm:max-w-[425px]">
				...
				<form onSubmit={handleSubmit}>
						...
						<div className="grid gap-2">
							...
							<div className="relative">
								<div className="flex gap-2">
									<div className="bg-primary/10 border-primary/20 text-primary inline-flex shrink-0 items-center rounded-md border px-2 py-1 text-sm font-medium">
								...
								{PLAYLIST_TEMPLATES.map((template, index) => {
									...
									return (
										<button
											...
											className={`rounded-lg border-2 p-3 text-left transition-all hover:scale-105 ${
												isSelected ?
					...
					<DialogFooter>
						...
						<Button type="submit" disabled={!isValidForm || isCreating}>
