...
import { AlertCircle, CheckCircle, EyeIcon, EyeOffIcon, Loader2 } from 'lucide-react'
		...
		const handleSaveApiKey = async () => {
			...
			try {
				...
				if (!isValid) {
					setLoading(null)
					showError(
						`Your key for ${getProviderDisplayName(activeProvider)} is invalid. Try copying it again from your provider.`
					)
					return false
				}
				...
				if (saveResponse.ok) {
					...
					showError(
						saveData.error ||
							`Failed to save ${getProviderDisplayName(activeProvider)} API key`
					)
					return false
				}
			} catch (error) {
		...
		return (
			<div className={`space-y-4 ${className}`}>
					...
					<div className="grid grid-cols-3 gap-2">
						{providers.map(providerName => (
							<button
								key={providerName}
								type="button"
								className={`flex items-center justify-start rounded-md border px-3 py-2 text-sm transition-colors ${activeProvider === providerName ? 'text-foreground border-blue-600 bg-blue-600' : 'text-muted-foreground/50 border-gray-300 bg-white hover:border-gray-400'}`}
				...
				<div className="space-y-2">
					...
					<div className="relative">
						<input
							...
							disabled={loading !== null}
							className="w-full rounded-md border border-gray-300 px-3 py-2 pr-10 shadow-sm focus:border-blue-500 focus:ring-blue-500 focus:outline-none sm:text-sm"
						/>
						<button
							type="button"
				...
				<div className="flex justify-end">
					<button
						type="button"
						className={`text-foreground inline-flex items-center rounded-md border border-transparent bg-blue-600 px-4 py-2 text-sm font-medium shadow-sm hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:outline-none ${!apiKey.trim() || loading !== null ? 'cursor-not-allowed opacity-75' : ''}`}
